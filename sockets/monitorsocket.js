...
console.log("monitor-location")
console.log('Connection established');
import {updateDbWithNewLocation} from './helpers.js';
import {db} from '../models/index.js'
const hoistedIOMonitor = (io, socket) => {
  console.log("monitorsocket"+ socket);
  return async function monitorLocation(payload) {
    console.log(
      `monitor-location event has been received with ${payload} 🍅🍋`
    );
    ...
    if (isOnline.dataValues.online) {
      ...
      if (recipient.trackerID) {
        ...
        io.to(socketID).emit("monitor-location", {
